...
function getMemoryFlag(): string {
  ...
  const memoryToUseString = process.env.SK_CODEQL_RAM;
...
async function createdDBForScannedLanguages(codeqlCmd: string, databaseFolder: string) {
  ...
  if (scannedLanguages) {
    for (const language of scannedLanguages.split(",")) {
      ...
      await exec.exec(
        codeqlCmd,
        ["resolve", "extractor", "--format=json", "--language=" + language],
        {
          silent: true,
          listeners: {
            stdout: (data) => {
      ...
      await exec.exec(
        ...
        {
          cwd: util.getRequiredEnvParam("CIRCLE_WORKING_DIRECTORY"),
        }
      );
    }
  }
...
async function finalizeDatabaseCreation(codeqlCmd: string, databaseFolder: string) {
  ...
  for (const language of languages.split(",")) {
    console.log("Finalizing " + language);
...
async function runResolveQueries(
  ...
  await exec.exec(codeqlCmd, ["resolve", "queries", ...queries, "--format=bylanguage"], options);

  return JSON.parse(output);
}

async function resolveQueryLanguages(
  codeqlCmd: string,
  ...
  if (config.additionalQueries.length !== 0) {
    ...
    if (noDeclaredLanguageQueries.length !== 0) {
      throw new Error(
        "Some queries do not declare a language, their qlpack.yml file is missing or is invalid"
      );
    }
    ...
    if (multipleDeclaredLanguagesQueries.length !== 0) {
      throw new Error(
        "Some queries declare multiple languages, their qlpack.yml file is missing or is invalid"
      );
    }
  }
...
async function runQueries(
  ...
  for (let database of fs.readdirSync(databaseFolder)) {
    ...
    if (queries.length === 0) {
      throw new Error(
        "Unable to analyse " + database + " as no queries were selected for this language"
      );
    }
    ...
    console.debug("Query suite file for " + database + "...\n" + querySuiteContents);
    ...
    console.debug("SARIF results for database " + database + ' created at "' + sarifFile + '"');
  }
}
  ...
  try {
    ...
    for (let report of files) {
      ...
      for (let result of run.results) {
        ...
        const artLoc = _.get(result, "locations.0.physicalLocation.artifactLocation");
        const artUri = artLoc.uri;
        let startLine = _.get(artLoc, "region.startLine");
...
export async function run() {
  ...
  try {
    ...
    process.env[sharedEnv.ODASA_TRACER_CONFIGURATION] = "";
    ...
    await util.mkdirP(outputFolder);

    console.info("Finalizing database creation");
